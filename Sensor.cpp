...
MaverickSensor::MaverickSensor(char * _strval) : Sensor(_strval)
{
    this->sensorClass = SENS_CLASS_OS;
    this->isValid = this->decode(_strval+4);
}
...
bool OregonSensorV2::decode_WGR918(char * pt)
{
    ...
    char dir[4];
    double idir; // direction en degres
    char speed[4];
    double ispeed; // vitesse en km/h
    char checksum[3];
    int ichecksum;
    char crc[3];
    int icrc;
