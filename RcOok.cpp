 ...
 *
 *
 *  MartinR63 on 17.03.2015
 *    Maverick BBQ Thermometer added based on https://forums.adafruit.com/viewtopic.php?f=8&t=25414
 *    Tested with Maverick ET-733
 *
...
void DecodeOOK::sprint(const char * s, char * d)
{
    ...
    for (byte i = 0; i < pos ; ++i)
    {
        sprintf(d,"%c",v[ data[i] >> 4 ]);
        d++;
        sprintf(d,"%c",v[ data[i] & 0x0F]);
        d++;
    }
    sprintf(d,'\0');

#ifdef TRACE_RCOOK
    std::cerr << " * DecodeOOK::sprint() - received [" << _d << "]" << std::endl;
#endif
}
 ...
 * ------------------------------------------------------
 */
MaverickDecoder::MaverickDecoder() {}

int MaverickDecoder::decode (word width)
{
    //printf("%d ",width);
    if (80 <= width && width < 5500)
    {
        if(width>4700 && state != UNKNOWN)
        {
            resetDecoder();
//             printf("reset\n");
        }
...
//                         printf("header found");
            ...
            else if (200<width<4700)
            {
                if (last)
                {
                    ++flip;
                    //printf("%d short pulse",flip);
                    last = 0;
                }
                else
                {
...
int RCSwitch_::decode (word duration)
{
       ...
       )
    {
        ...
        {
        case 1:
            if (receiveProtocol1(_changeCount) == true) return 1;
            break;
        case 2:
            if (receiveProtocol2(_changeCount) == true) return 1;
            break;
        }
        return -1;
